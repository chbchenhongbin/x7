...
public class JedisConnector_Cache {
	...
	public static JedisConnector_Cache getInstance(){
		if (instance == null){
			...
			config.setMaxTotal(Configs.getIntValue("REDIS_MAX_CACHE"));
			config.setMaxIdle(Configs.getIntValue("REDIS_IDLE_CACHE"));
			config.setJmxEnabled(true);
			config.setJmxNamePrefix("redis-cahce");
			config.setTestOnBorrow(true);
			
			pool = new JedisPool( config, Configs.getString("REDIS_IP_CACHE"), Configs.getIntValue("REDIS_PORT_CACHE"));  //6379
		}
		return instance;
	}
